...
def conectar():
    ...
    """
    try:
        conn = MySQLdb.connect(db='roster', host='localhost', user='root', password='')
        return conn
...
def listar():
    ...
    produtos = cursor.fetchall()  # Vai pegar o resultado do comando anterior e transformar em uma lista!
    if len(produtos) > 0:
        print('Listando produtos ...')
        print('-' * 20)
        for produto in produtos:
            print(f'ID: {produto[0]}')
            print(f'Tipo: {produto[1]}')
            print(f'Preço: {produto[2]}')
            print(f'Estoque: {produto[3]}')
            print('-' * 20)
    else:
        print('Não existem produtos cadastradas!')
    desconectar(conn)
